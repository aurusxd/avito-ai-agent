<script lang="ts">
	...
	const activeCount = $derived(data.accounts.filter((item) => item.status === 'active').length);
	const bannedCount = $derived(data.accounts.filter((item) => item.status === 'banned').length);
	const rotationIds = $derived(
		new Set((data.rotation?.members ?? []).filter((m) => m.in_rotation).map((m) => m.account_id))
	);
...
<div class="space-y-6">
	<div class="grid gap-4 sm:grid-cols-4">
		...
		<StatCard label="Активных" value={activeCount} trend="up" hint="участвуют в отправке" />
		<StatCard label="Заблокировано" value={bannedCount} trend={bannedCount ? 'down' : 'flat'} />
		<StatCard
		...
		<Card.Root>
			<Card.Header>
				...
				<Card.Description>
					...
					{data.rotation.delay_min_minutes}–{data.rotation.delay_max_minutes} мин, лимит
			...
			<Card.Content>
				{#if data.rotation.next_account_id}
					...
					<p class="text-sm">
						Следующим пишет <span class="font-medium">{next?.login}</span>, у него осталось
						{next?.remaining_today} сообщений.
					</p>
				{:else}
					<p class="text-muted-foreground text-sm">
						Свободных аккаунтов нет: лимиты выбраны или все аккаунты вне работы.
					</p>
				{/if}
			</Card.Content>
		</Card.Root>
	...
	<Card.Root>
		<Card.Header>
			...
			<Card.Description>
				Авторизация только через файл сессии Playwright, пароли панель не хранит
			</Card.Description>
			<Card.Action>
				<Dialog.Root bind:open={createOpen}>
					...
					<Dialog.Content class="sm:max-w-md">
						<Dialog.Header>
							...
							<Dialog.Description>
								Сначала сохраните сессию скриптом входа, затем укажите путь к файлу
							</Dialog.Description>
						</Dialog.Header>
						<form
							method="POST"
						...
						>
							...
							<div class="space-y-2">
								<Label for="session_storage_path">Путь к файлу сессии</Label>
								<Input
				...
				<Table.Root>
					<Table.Header>
						<Table.Row>
							...
							<Table.Head class="text-right">Лимит</Table.Head>
					...
					<Table.Body>
						{#each data.accounts as account (account.id)}
							<Table.Row>
								<Table.Cell class="font-medium">{account.login}</Table.Cell>
								<Table.Cell>
									<Badge variant={account.has_session ? 'default' : 'destructive'}>
										{account.has_session ? 'найдена' : 'нет файла'}
									</Badge>
								</Table.Cell>
								...
								<Table.Cell class="text-right">
									...
									>
										...
										<Input
											...
											aria-label={'Суточный лимит для ' + account.login}
										/>
								...
								<Table.Cell>
									{#if rotationIds.has(account.id)}
										<Badge variant="outline">в ротации</Badge>
									{:else}
										<span class="text-muted-foreground text-sm">резерв</span>
									{/if}
								</Table.Cell>
								<Table.Cell>
									<div class="flex items-center gap-2">
										...
										>
											...
											<Switch
												...
												aria-label={'Переключить аккаунт ' + account.login}
												onCheckedChange={() => toggleForms[account.id]?.requestSubmit()}
											/>
										</form>
